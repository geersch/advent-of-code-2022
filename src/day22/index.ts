...
function solve1(moves: string): number {
 ...
 *└──────────┘
 */
function solve2(moves: string): number {
  const matches = moves.matchAll(/(?<count>\d+)(?<dir>[RL]?)/g);
  for (const match of matches) {
    const { count, dir } = match.groups!;
    const tiles = parseInt(count);
...
console.log(`The final password is ${solve1(moves)}.`);
console.log(`After folding the map into a cube, the final password is ${solve2(moves)}.`);
